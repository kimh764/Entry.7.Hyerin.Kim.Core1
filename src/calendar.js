    ...
    for(let i=1; i<=dayCount; i++){
        if(i===currentDay&&year===currentYear&&mon===currentMon){
            ...
            if(currentYear === yearOfChoice && currentMon === MonOfChoice && currentDay === DayOfChoice){
                onlyOneList.style.border = '4px solid pink';
            }
            else{
                onlyOneList.style.border = '4px solid gray';
            }

            if(0===getDay(year,mon,i)){
                onlyOneList.style.color = 'pink';
            }
        ...
        if(i===DayOfChoice&&year===yearOfChoice&&mon===MonOfChoice){
            list.style.border = '4px solid pink';
            Day.addEventListener('click',(event)=>{
...
function main(){
    setMonthTitle(year,mon);
    makeCalendar(year,mon,getDayOfMon(mon,year));
    todoTitle.textContent = `What are you going to travel on ${year}.${mon}.${currentDay}😍⁉`;
    displayToDoOnDays();
}
...
Day.addEventListener('click',(event)=>{
    ...
    if(event.target.className!=='disabled'){
        clearEvent();
        todoTitle.textContent = `Do you meet travel companion on ${year}.${mon}.${event.target.textContent} 🗺️⁉`;
...
reset.addEventListener('click',()=>{
    const result = prompt(`✈️Do you meet your friends on ${year} ${mon} ${DayOfChoice}? Enter (y/n)`);
    const YMD = year+'-'+mon+'-'+DayOfChoice;
    if(result==='y'){
...
allReset.addEventListener('click',()=>{
    const result = prompt(` Enter travel preparation (y/n) recomended✨`);
    if(result==='y'){
        localStorage.clear();
        displayToDoOnDays();
    }
